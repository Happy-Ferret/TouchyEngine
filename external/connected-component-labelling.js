function cclb(){
     ...
     * @param length 
     */
    function memset(arraytoset, offset, length, value) {
	    for (var i = 0; i < length; i++) {
		    arraytoset[offset++] = value;
	    }
        return arraytoset
    };
    ...
    this.eBlobColouring = function(dest, width, height, labels){
        return BlobColouring(dest, width, height, labels)
    }

    this.eBlobExtraction = function(data, w, h){
        return BlobExtraction(data, w, h)
    }

    this.eBlobBounds = function(data, w, h){
        return BlobBounds(data, w, h)
    }
